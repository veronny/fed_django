            ...
            cursor.execute(
                '''
                    ...
                    CASE 
                        ...
                        , 2) 
                    END AS cob
                    FROM public."SI_0301_CRED12M_Combinado"
					WHERE "año" = '2025'
                '''
            )
...
def obtener_avance_regional_mensual_s3_cred12(anio):
    """
...
def index_s3_cred12(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        try:
            ...
            # Obtener datos de AVANCE GRAFICO ANUAL
            resultados_avance_regional_mensual_s3_cred12 = obtener_avance_regional_mensual_s3_cred12(anio)
...
def get_microredes_s3_cred12(request, microredes_id):
    redes = (
            MAESTRO_HIS_ESTABLECIMIENTO
            .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL',Departamento='JUNIN')
            .annotate(codigo_red_filtrado=Substr('Codigo_Red', 1, 4))
    ...
    return render(request, 's3_cred12/microredes.html', context)

def p_microredes_s3_cred12(request):
    ...
    return render(request, 's3_cred12/partials/p_microredes.html', context)

## REPORTE POR ESTABLECIMIENTO
def get_establecimientos_s3_cred12(request,establecimiento_id):
    redes = (
                MAESTRO_HIS_ESTABLECIMIENTO
    ...
    return render(request,'s3_cred12/establecimientos.html', context)

def p_microredes_establec_s3_cred12(request):
    redes_param = request.GET.get('red') 
    microredes = MAESTRO_HIS_ESTABLECIMIENTO.objects.filter(Codigo_Red=redes_param, Descripcion_Sector='GOBIERNO REGIONAL',Disa='JUNIN').values('Codigo_MicroRed','MicroRed').distinct()
    context = {
        'microredes': microredes,
    ...
    return render(request, 's3_cred12/partials/p_microredes_establec.html', context)

def p_establecimientos_s3_cred12(request):
    ...
    return render(request, 's3_cred12/partials/p_establecimientos.html', context)

# query 
def obtener_seguimiento_redes_s3_cred12(p_anio,p_red,p_microred,p_establec,p_inicio,p_fin,p_cumple):
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT * FROM public.fn_seguimiento_s3_cred12(%s,%s,%s,%s,%s,%s,%s)",
...
class RptS3CredRed(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        resultado_seguimiento = obtener_seguimiento_redes_s3_cred12(p_anio,p_red,p_microred,p_establec,p_inicio,p_fin,p_cumple)
...
class RptS3CredMicroRed(TemplateView):
    def get(self, request, *args, **kwargs):
        # Variables ingresadas
        p_anio = request.GET.get('anio')
        p_red = request.GET.get('red','')
        ...
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_s3_cred12_red.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
...
class RptS3CredEstablec(TemplateView):
    def get(self, request, *args, **kwargs):
        # Variables ingresadas
        p_anio = request.GET.get('anio')
        p_red = request.GET.get('red','')
