...
## AVANCE REGIONAL
def obtener_avance_regional_s1_gestante_anemia():
    """
...
def obtener_avance_regional_mensual_s1_gestante_anemia():
    """
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                '''
                    ...
                    FROM 
                    public."Cobertura_SI_01_AnemiaGestante";
                '''
            )
            resultados = cursor.fetchall()
...
def index_s1_gestante_anemia(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        try:
            ...
            resultados_avance_regional_s1_gestante_anemia = obtener_avance_regional_s1_gestante_anemia()
            # Obtener datos de AVANCE GRAFICO ANUAL
            resultados_avance_regional_mensual_s1_gestante_anemia = obtener_avance_regional_mensual_s1_gestante_anemia()
            ...
            for index, row in enumerate(resultados_avance_regional_s1_gestante_anemia):
                try:
            ...
            for index, row in enumerate(resultados_avance_regional_mensual_s1_gestante_anemia):
                try:
                    # Verifica que el diccionario tenga las claves necesarias
...
## REPORTE POR MICROREDES
def get_microredes_s1_gestante_anemia(request, microredes_id):
    redes = (
            MAESTRO_HIS_ESTABLECIMIENTO
            .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL',Departamento='JUNIN')
            .annotate(codigo_red_filtrado=Substr('Codigo_Red', 1, 4))
    ...
    return render(request, 's1_gestante_anemia/microredes.html', context)

def p_microredes_s1_gestante_anemia(request):
    ...
    return render(request, 's1_gestante_anemia/partials/p_microredes.html', context)

## REPORTE POR ESTABLECIMIENTO
def get_establecimientos_s1_gestante_anemia(request,establecimiento_id):
    redes = (
                MAESTRO_HIS_ESTABLECIMIENTO
    ...
    return render(request,'s1_gestante_anemia/establecimientos.html', context)

def p_microredes_establec_s1_gestante_anemia(request):
    redes_param = request.GET.get('red') 
    microredes = MAESTRO_HIS_ESTABLECIMIENTO.objects.filter(Codigo_Red=redes_param, Descripcion_Sector='GOBIERNO REGIONAL',Disa='JUNIN').values('Codigo_MicroRed','MicroRed').distinct()
    context = {
        'microredes': microredes,
    ...
    return render(request, 's1_gestante_anemia/partials/p_microredes_establec.html', context)

def p_establecimientos_s1_gestante_anemia(request):
    ...
    return render(request, 's1_gestante_anemia/partials/p_establecimientos.html', context)

## query funcion seguimiento 
def obtener_seguimiento_redes_s1_gestante_anemia(p_anio,p_red,p_microred,p_establec,p_inicio,p_fin,p_cumple):
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT * FROM public.fn_seguimiento_s1_gestante_anemia(%s,%s,%s,%s,%s,%s,%s)",
...
class RptS1GestanteAnemiaRed(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        resultado_seguimiento = obtener_seguimiento_redes_s1_gestante_anemia(p_anio,p_red,p_microred,p_establec,p_inicio,p_fin,p_cumple)
...
class RpS1GestanteAnemiaMicroRed(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_s1_gestante_anemia_microred.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
...
class RptS1GestanteAnemiaEstablec(TemplateView):
    def get(self, request, *args, **kwargs):
        # Variables ingresadas
        p_anio = request.GET.get('anio')
        p_red = request.GET.get('red','')
        ...
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_s1_gestante_anemia_establec.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
...
###########################################################################################
# -- COBERTURA GESTANTE CON ANEMIA
def obtener_cobertura_s1_gestante_anemia():
    with connection.cursor() as cursor:
        cursor.execute(
            'SELECT * FROM public."Cobertura_SI_01_AnemiaGestante" ORDER BY "Red", "MicroRed", "Nombre_Establecimiento";'
        )
        return cursor.fetchall()

class RptCoberturaS1GestanteAnemia(TemplateView):
    def get(self, request, *args, **kwargs):
        # Variables ingresadas
                
        # Creación de la consulta
        resultado_cobertura = obtener_cobertura_s1_gestante_anemia()
        ...
        for index, (sheet_name, results) in enumerate(consultas):
            ...
            fill_worksheet_cobertura_s1_gestante_anemia(ws, results)
        
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_cobertura_s1_gestante_anemia.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
...
def fill_worksheet_cobertura_s1_gestante_anemia(ws, results): 
    # cambia el alto de la columna
    ws.row_dimensions[1].height = 14
    ws.row_dimensions[2].height = 14
    ...
    ws['B4'].alignment = Alignment(horizontal= "left", vertical="center")
    ws['B4'].font = Font(name = 'Arial', size= 12, bold = True)
    ws['B4'] = 'COBERURA DEL INDICADOR SI-01. PORCENTAJE DE GESTANTES CON DIAGNÓSTICO DE ANEMIA ATENDIDAS EN ESTABLECIMIENTOS DE SALUD DEL PRIMER Y SEGUNDO NIVEL DE ATENCIÓN, QUE RECIBEN DOSAJE DE HEMOGLOBINA DE CONTROL Y SEGUNDA ENTREGA DE TRATAMIENTO CON HIERRO CON DIAGNÓSTO DE ANEMIA'
    ...
    ws['E8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'
    
    ws['F8'].alignment = Alignment(horizontal= "center", vertical="center", wrap_text=True)
    ws['F8'].font = Font(name = 'Arial', size= 7, bold = True, color='000000')
    ws['F8'].fill = yellow_fill
    ws['F8'].border = border_negro
    ws['F8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo'
    ...
    ws['H8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'    
    
    ws['I8'].alignment = Alignment(horizontal= "center", vertical="center", wrap_text=True)
    ws['I8'].font = Font(name = 'Arial', size= 7, bold = True, color='000000')
    ws['I8'].fill = blue_fill
    ws['I8'].border = border_negro
    ws['I8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo' 
    ...
    ws['K8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'     
    
    ws['L8'].alignment = Alignment(horizontal= "center", vertical="center", wrap_text=True)
    ws['L8'].font = Font(name = 'Arial', size= 7, bold = True, color='000000')
    ws['L8'].fill = blue_fill
    ws['L8'].border = border_negro
    ws['L8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo' 
    ...
    ws['N8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'   
    
    ws['O8'].alignment = Alignment(horizontal= "center", vertical="center", wrap_text=True)
    ws['O8'].font = Font(name = 'Arial', size= 7, bold = True, color='000000')
    ws['O8'].fill = blue_fill
    ws['O8'].border = border_negro
    ws['O8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo'
    ...
    ws['Q8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'   
    
    ws['R8'].alignment = Alignment(horizontal= "center", vertical="center", wrap_text=True)
    ws['R8'].font = Font(name = 'Arial', size= 7, bold = True, color='000000')
    ws['R8'].fill = blue_fill
    ws['R8'].border = border_negro
    ws['R8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo' 
    ...
    ws['T8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'    
    
    ws['U8'].alignment = Alignment(horizontal= "center", vertical="center", wrap_text=True)
    ws['U8'].font = Font(name = 'Arial', size= 7, bold = True, color='000000')
    ws['U8'].fill = blue_fill
    ws['U8'].border = border_negro
    ws['U8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo'
    ...
    ws['W8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'   
    ...
    ws['X8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo'
    ...
    ws['Z8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'   
    ...
    ws['AA8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo'
    ...
    ws['AC8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'   
    ...
    ws['AD8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo'
    ...
    ws['AF8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'   
    ...
    ws['AG8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo' 
    ...
    ws['AI8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'    
    ...
    ws['AJ8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo'
    ...
    ws['AL8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'    
    ...
    ws['AM8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo'
    ...
    ws['AO8'] = 'N° de gestantes del denominador que reciben un dosaje de hemoglobina de control y segunda entrega de hierro al mes de iniciado el tratamiento de anemia, registrados en el HIS'   
    ...
    ws['AP8'] = 'N° de gestantes atendidas en establecimientos de salud del 1er y 2do nivel de atención que cuentan con diagnóstico de anemia y primera entrega de hierro, registrados en HIS, en el mes previo' 
