...
## AVANCE REGIONAL
def obtener_avance_regional_s4_adolescente_dosaje():
    """
    ...
    try:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                FROM public."Cobertura_SI_04_AdolescenteDosaje"
                '''
            )
...
def obtener_avance_regional_mensual_s4_adolescente_dosaje(anio):
    """
            ...
            cursor.execute(
                '''
                    ...
                    CASE 
                        ...
                        , 2) 
                    END AS COB_12
                    FROM public."SI_04_AdolescenteDosaje_Combinado"
                    WHERE año = %s;
                ''',
                [anio]  # Filtro por año
            )
...
def index_s4_adolescente_dosaje(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        try:
            ...
            resultados_avance_regional_s4_adolescente_dosaje = obtener_avance_regional_s4_adolescente_dosaje()
            # Obtener datos de AVANCE GRAFICO ANUAL
            resultados_avance_regional_mensual_s4_adolescente_dosaje = obtener_avance_regional_mensual_s4_adolescente_dosaje(anio)
            ...
            for index, row in enumerate(resultados_avance_regional_s4_adolescente_dosaje):
                try:
            ...
            for index, row in enumerate(resultados_avance_regional_mensual_s4_adolescente_dosaje):
                try:
                    # Verifica que el diccionario tenga las claves necesarias
...
def get_microredes_s4_adolescente_dosaje(request, microredes_id):
    redes = (
            MAESTRO_HIS_ESTABLECIMIENTO
            .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL',Departamento='JUNIN')
            .annotate(codigo_red_filtrado=Substr('Codigo_Red', 1, 4))
    ...
    return render(request, 's4_adolescente_dosaje/microredes.html', context)

def p_microredes_s4_adolescente_dosaje(request):
    ...
    return render(request, 's4_adolescente_dosaje/partials/p_microredes.html', context)

## REPORTE POR ESTABLECIMIENTO
def get_establecimientos_s4_adolescente_dosaje(request,establecimiento_id):
    redes = (
                MAESTRO_HIS_ESTABLECIMIENTO
    ...
    return render(request,'s4_adolescente_dosaje/establecimientos.html', context)

def p_microredes_establec_s4_adolescente_dosaje(request):
    redes_param = request.GET.get('red') 
    microredes = MAESTRO_HIS_ESTABLECIMIENTO.objects.filter(Codigo_Red=redes_param, Descripcion_Sector='GOBIERNO REGIONAL',Disa='JUNIN').values('Codigo_MicroRed','MicroRed').distinct()
    context = {
        'microredes': microredes,
    ...
    return render(request, 's4_adolescente_dosaje/partials/p_microredes_establec.html', context)

def p_establecimientos_s4_adolescente_dosaje(request):
    ...
    return render(request, 's4_adolescente_dosaje/partials/p_establecimientos.html', context)


## QUERY 
def obtener_seguimiento_redes_s4_adolescente_dosaje(p_anio,p_red,p_microred,p_establec,p_inicio,p_fin,p_cumple):
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT * FROM public.fn_seguimiento_s4_adolescente_dosaje(%s,%s,%s,%s,%s,%s,%s)",
...
class RptS4AdolescenteDosajeRed(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        resultado_seguimiento = obtener_seguimiento_redes_s4_adolescente_dosaje(p_anio,p_red,p_microred,p_establec,p_inicio,p_fin,p_cumple)
...
class RptS4AdolescenteDosajeMicroRed(TemplateView):
    def get(self, request, *args, **kwargs):
        # Variables ingresadas
        p_anio = request.GET.get('anio')
        p_red = request.GET.get('red','')
        ...
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_s4_adolescente_dosaje_red.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
...
class RptS4AdolescenteDosajeEstablec(TemplateView):
    def get(self, request, *args, **kwargs):
        # Variables ingresadas
        p_anio = request.GET.get('anio')
        p_red = request.GET.get('red','')
...
def fill_worksheet_s4_adolescente_dosaje(ws, results): 
    ...
    ws['C6'] = 'DENOMINADOR'
    ws['F6'] = 'NUMERADOR'
    ws['C7'] = 'Adolescentes atendidas (N y R) en el EESS y mes de medicion'
    ws['F7'] = '(DX=85018 + TD = D) ó (DX=85018.01 + TD = D)'
