...
def obtener_avance_regional_mensual_s21_suplementacion6(anio):
    """
            ...
            cursor.execute(
                '''
                    ...
                    CASE 
                        ...
                        , 2) 
                    END AS COB_12
                    FROM public."SI_0201_Suplementacion6M_Combinado"
                    WHERE año = %s;
                ''',
                [anio]  # Filtro por año
            )
...
def index_s21_suplementacion6(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        try:
            ...
            # Obtener datos de AVANCE GRAFICO ANUAL
            resultados_avance_regional_mensual_s21_suplementacion6 = obtener_avance_regional_mensual_s21_suplementacion6(anio)
...
def get_microredes_s21_suplementacion6(request, microredes_id):
    redes = (
            MAESTRO_HIS_ESTABLECIMIENTO
            .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL',Departamento='JUNIN')
            .annotate(codigo_red_filtrado=Substr('Codigo_Red', 1, 4))
    ...
    return render(request, 's21_suplementacion6/microredes.html', context)

def p_microredes_s21_suplementacion6(request):
    ...
    return render(request, 's21_suplementacion6/partials/p_microredes.html', context)

## REPORTE POR ESTABLECIMIENTO
def get_establecimientos_s21_suplementacion6(request,establecimiento_id):
    redes = (
                MAESTRO_HIS_ESTABLECIMIENTO
    ...
    return render(request,'s21_suplementacion6/establecimientos.html', context)

def p_microredes_establec_s21_suplementacion6(request):
    redes_param = request.GET.get('red') 
    microredes = MAESTRO_HIS_ESTABLECIMIENTO.objects.filter(Codigo_Red=redes_param, Descripcion_Sector='GOBIERNO REGIONAL',Disa='JUNIN').values('Codigo_MicroRed','MicroRed').distinct()
    context = {
        'microredes': microredes,
    ...
    return render(request, 's21_suplementacion6/partials/p_microredes_establec.html', context)

def p_establecimientos_s21_suplementacion6(request):
    ...
    return render(request, 's21_suplementacion6/partials/p_establecimientos.html', context)


## QUERY
def obtener_seguimiento_redes_s21_suplementacion6(p_anio,p_red,p_microred,p_establec,p_inicio,p_fin,p_cumple):
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT * FROM public.fn_seguimiento_s21_suplementacion6(%s,%s,%s,%s,%s,%s,%s)",
...
class RptS21Suplementacion6Red(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        resultado_seguimiento = obtener_seguimiento_redes_s21_suplementacion6(p_anio,p_red,p_microred,p_establec,p_inicio,p_fin,p_cumple)
...
class RptS21Suplementacion6MicroRed(TemplateView):
    def get(self, request, *args, **kwargs):
        # Variables ingresadas
        p_anio = request.GET.get('anio')
        p_red = request.GET.get('red','')
        ...
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_s21_suplementacion6_red.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
...
class RptS21Suplementacion6Establec(TemplateView):
    def get(self, request, *args, **kwargs):
        # Variables ingresadas
        p_anio = request.GET.get('anio')
        p_red = request.GET.get('red','')
...
def fill_worksheet_s21_suplementacion6(ws, results): 
    ...
    ws['J8'] = 'DX = 99199.17'
    ws['L8'] = 'DX = 85018 ó 85018.01 + TD = D'
    ws['N8'] = 'DX = 99199.17'
    ws['P8'] = 'DX = 85018 ó 85018.01 + TD = D'
    ws['S8'] = 'DX = 99199.17'
    ws['U8'] = 'DX = 85018 ó 85018.01 + TD = D '
    ### numerador y denominador 
...
def obtener_cobertura_s21_suplementacion6():
    with connection.cursor() as cursor:
        cursor.execute(
            'SELECT * FROM public."Cobertura_SI_0201_Suplementacion6M" ORDER BY "Red", "MicroRed", "Nombre_Establecimiento";'
        )
        return cursor.fetchall()

class RptCoberturaS21Suplementacion6(TemplateView):
    def get(self, request, *args, **kwargs):
        # Variables ingresadas
                
        # Creación de la consulta
        resultado_cobertura = obtener_cobertura_s21_suplementacion6()
        ...
        for index, (sheet_name, results) in enumerate(consultas):
            ...
            fill_worksheet_cobertura_s21_suplementacion6(ws, results)
        
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_cobertura_s21_suplementacion6.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
...
def fill_worksheet_cobertura_s21_suplementacion6(ws, results): 
    # cambia el alto de la columna
    ws.row_dimensions[1].height = 14
    ws.row_dimensions[2].height = 14
    ws.row_dimensions[3].height = 12
    ...
    ws['B4'].alignment = Alignment(horizontal= "left", vertical="center")
    ws['B4'].font = Font(name = 'Arial', size= 12, bold = True)
    ws['B4'] = 'COBERTURA DEL INDICADOR SI-02.01. PORCENTAJE DE NIÑAS Y NIÑOS MESES DE EDAD DEL DEPARTAMENTO, DE SEIS (06) MESES DE EDAD, PREMATUROS Y/O BAJO PESO AL NACER, Y DE CUATRO (04) SIN DIAGNÓSTICO DE ANEMIA; QUE RECIBEN DOSAJES DE HEMOGLOBINA Y CULMINAN LA SUPLEMENTACIÓN PREVENTIVA CON HIERRO'
