...
def obtener_avance_v2_tamizaje_violencia(red):
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT * FROM public.obtener_avance_v2_tamizaje_violencia(%s)",
            [red]
        )
...
def obtener_ranking_v2_tamizaje_violencia(anio, mes):
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT * FROM public.obtener_ranking_v2_tamizaje_violencia(%s, %s)",
            [anio, mes]
        )
...
## AVANCE REGIONAL
def obtener_avance_regional_v2_tamizaje_violencia():
    """
    ...
    try:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                FROM public."Cobertura_VI0102_TamizajeViolenciaGestante"
                '''
            )
...
def obtener_avance_regional_mensual_v2_tamizaje_violencia(anio):
    """
            ...
            cursor.execute(
                '''
                    ...
                    CASE 
                        ...
                        , 2) 
                    END AS COB_12
                    FROM public."VI0102_TamizajeViolenciaGestante_Combinado"
                    WHERE año = %s;
                ''',
                [anio]  # Filtro por año
            )
...
def index_v2_tamizaje_violencia(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        try:
            ...
            resultados_avance_regional_v2_tamizaje_violencia = obtener_avance_regional_v2_tamizaje_violencia()
            # Obtener datos de AVANCE GRAFICO ANUAL
            resultados_avance_regional_mensual_v2_tamizaje_violencia = obtener_avance_regional_mensual_v2_tamizaje_violencia(anio)
            ...
            for index, row in enumerate(resultados_avance_regional_v2_tamizaje_violencia):
                try:
            ...
            for index, row in enumerate(resultados_avance_regional_mensual_v2_tamizaje_violencia):
                try:
                    # Verifica que el diccionario tenga las claves necesarias
...
def fill_worksheet_v2_tamizaje_violencia(ws, results): 
    ...
    ws['C6'] = 'Atencion Prenatal'
    ws['E6'] = 'Registro de Detección de violencia contra la mujer en gestantes'
    ws['G6'] = 'Tamizaje positivo: Problemas relacionados con la violencia'
    
    ws['C7'] = 'DX=Z3491 ó Z3591 ó Z3492 ó Z3592 ó Z3493 ó Z3593'
    ws['E7'] = 'DX= 96150.01 + TD = D'
    ws['F7'] = 'DX= 96150 + TD = D + LAB = VIF'
    ws['G7'] = 'DX= R456 + TD = D'
    ws['H7'] = 'DX= R456 + TD = D + LAB = VIF'
    ...
    ws['G5'].fill = naranja_claro_fill
    ws['G5'].border = border_negro
...
def obtener_cobertura_v2_tamizaje_violencia():
    with connection.cursor() as cursor:
        cursor.execute(
            'SELECT * FROM public."Cobertura_VI0102_TamizajeViolenciaGestante" ORDER BY "Red", "MicroRed", "Nombre_Establecimiento";'
        )
        return cursor.fetchall()
...
class RptCoberturaV2TamizajeViolencia(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        for index, (sheet_name, results) in enumerate(consultas):
            ...
            fill_worksheet_cobertura_v2_tamizaje_violencia(ws, results)
        
        ##########################################################################          
        # Establecer el nombre del archivo
        nombre_archivo = "rpt_cobertura_v2_tamizaje_violencia.xlsx"
        # Definir el tipo de respuesta que se va a dar
        response = HttpResponse(content_type="application/ms-excel")
        contenido = "attachment; filename={}".format(nombre_archivo)
        response["Content-Disposition"] = contenido
...
def fill_worksheet_cobertura_v2_tamizaje_violencia(ws, results): 
    # cambia el alto de la columna
    ws.row_dimensions[1].height = 14
    ws.row_dimensions[2].height = 14
    ...
    ws['B4'].alignment = Alignment(horizontal= "left", vertical="center")
    ws['B4'].font = Font(name = 'Arial', size= 12, bold = True)
    ws['B4'] = 'COBERURA DEL INDICADOR VI-01.02. PORCENTAJE DE GESTANTES ATENDIDAS EN ESTABLECIMIENTOS DE SALUD DEL PRIMER Y SEGUNDO NIVEL DE ATENCIÓN DEL GOBIERNO REGIONAL, QUE CUENTAN TAMIZAJE POSITIVO DE VIOLENCIA CONTRA LA MUJER'
    ...
    ws['F8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud'
    ...
    ws['I8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud' 
    ...
    ws['L8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud' 
    ...
    ws['O8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud'
    ...
    ws['R8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud' 
    ...
    ws['U8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud'
    ...
    ws['X8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud'
    ...
    ws['AA8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud'
    ...
    ws['AD8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud'
    ...
    ws['AG8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud' 
    ...
    ws['AJ8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud'
    ...
    ws['AM8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud'
    ...
    ws['AP8'] = 'N° de gestantes a quienes le aplicaron la ficha de detección de violencia contra la mujer, en Establecimientos de Salud' 
